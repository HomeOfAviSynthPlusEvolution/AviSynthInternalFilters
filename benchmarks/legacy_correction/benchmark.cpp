...
void measure(Runtime& runtime, const char* plugin, bool migrated) {
  Environment holder(runtime);
  auto* env = holder.env;
  if (migrated)
    env->Invoke("LoadPlugin", plugin);
  for (int type : {VideoInfo::CS_YUY2}) {
    PClip a(new Sequence(env, type, 960, 540, false));
    for (const char* name : {"FixLuminance", "FixBrokenChromaUpsampling", "PeculiarBlend", "SkewRows"}) {
      std::vector<AVSValue> args = {a};
      if (std::strcmp(name, "FixLuminance") == 0) {
        args.push_back(180);
        args.push_back(1.0);
      }
      if (std::strcmp(name, "PeculiarBlend") == 0)
        args.push_back(270);
      if (std::strcmp(name, "SkewRows") == 0)
        args.push_back(2);
      std::vector<double> times;
      for (int i = 0; i < 17; ++i) {
        auto start = std::chrono::steady_clock::now();
        PClip f = env->Invoke(name, AVSValue(args.data(), int(args.size()))).AsClip();
        for (int n = 0; n < 5; ++n) {
          auto frame = f->GetFrame(n, env);
          (void)frame->GetReadPtr()[0];
        }
        auto end = std::chrono::steady_clock::now();
        if (i >= 2)
          times.push_back(std::chrono::duration<double, std::milli>(end - start).count() / 5);
      }
      std::sort(times.begin(), times.end());
      std::printf("%s,%d,%s,%.6f\n", name, type, migrated ? "new" : "main", times[times.size() / 2]);
    }
  }
}
} // namespace
int main(int argc, char** argv) {
  ...
  if (!runtime.module)
    return 2;
  try {
    measure(runtime, argv[2], false);
    measure(runtime, argv[2], true);
