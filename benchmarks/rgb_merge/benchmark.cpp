    ...
    for (int bytes : {1, 2})
      for (int sc : {1, 2, 3, 4})
        for (int dc : {1, 3, 4}) {
          if (bytes == 2 && sc == 2)
            continue;
          const int h = w == 640 ? 360 : 1080, op = sc * 10 + dc;
          ...
          auto apply = [&](uint8_t* dst, uint32_t cpu) {
            ...
            const uint8_t* sources[] = {src.data(), src.data(), src.data(), sc == 3 ? nullptr : src.data()};
            const int source_pitches[] = {sp, sp, sp, sp}, kinds[] = {sc, sc, sc, sc};
            return aif_rgb_merge_render(sources, source_pitches, kinds, p, pitches, w, h, bytes, dc, cpu);
          };
          ...
          for (const auto& t : targets) {
            if (t.mask && t.mask != ~0u && !(t.mask & aif_rgb_merge_supported_cpu()))
              continue;
