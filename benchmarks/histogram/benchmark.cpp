...
void measure(Runtime& runtime, const char* plugin, bool migrated) {
  ...
  for (int type : {VideoInfo::CS_YV12, VideoInfo::CS_YUV444P16, VideoInfo::CS_YUV444PS}) {
    PClip a(new Sequence(env, type, 960, 540, false)), b(new Sequence(env, type, 960, 540, true));
    for (const char* name : {"Levels", "Color2", "Luma"}) {
      const AVSValue args[] = {a, name};
      std::vector<double> times;
      for (int i = 0; i < 17; ++i) {
        auto start = std::chrono::steady_clock::now();
        PClip f = env->Invoke("Histogram", AVSValue(args, 2)).AsClip();
        for (int n = 0; n < 5; ++n) {
