...
int main() {
  ...
  std::puts("width,height,bytes,unaligned,target,median_us");
  for (int w : {960, 1920})
    for (int bytes : {1, 2, 3, 4, 6, 8})
      for (int op : {0, 1}) {
        const int h = w == 960 ? 540 : 1080;
        const int row = w * bytes, pitch = row + op;
        ...
        auto apply = [&](uint8_t* dst, uint32_t cpu) {
          return aif_blank_clip_fill(dst + op, pitch, row, h, pattern, bytes, cpu);
        };
        ...
        for (const auto& t : targets) {
          if (t.mask && t.mask != ~0u && !(t.mask & aif_blank_clip_supported_cpu()))
            continue;
