...
struct Scratch {
  ...
  Scratch(int width, int height, int bytes)
      : storage((size_t(width) * height * bytes + 3) / 4), plane{storage.data(), ptrdiff_t(width) * bytes, bytes} {}
};
...
void LayerFrame(PVideoFrame& base, const PVideoFrame& source, const VideoInfo& vi, const VideoInfo& source_vi,
                ...
                int placement, IScriptEnvironment* env, const cp_kernels* k) {
  ...
  config.operation = operation == LayerOperation::Subtract   ? CP_INVERT_MIX
                     ...
                     : operation == LayerOperation::Darken   ? CP_SELECT_DARKER
                                                             : CP_MIX;
  ...
  if (vi.IsRGB()) {
    ...
    for (int c = 0; c < count; ++c) {
      ...
      src[c] = Offset(cp_const_plane{source->GetReadPtr(p) + offset, source->GetPitch(p), step}, overlap.source_x,
                      overlap.source_y);
    }
    ...
    for (int c = 0; c < channels; ++c)
      Check(k->process_plane(&config, Read(dst[c]), chroma ? src[c] : sg, alpha && !fast ? &src[3] : nullptr,
                             select ? &bg : nullptr, select ? &sg : nullptr, dst[c], rows),
            env);
    return;
  }
  ...
  const auto source_alpha =
      alpha ? cp_const_plane{source->GetReadPtr(PLANAR_A), source->GetPitch(PLANAR_A), bytes} : cp_const_plane{};
  ...
  for (int index = 0; index < count; ++index) {
    ...
    cp_plane dst = Offset(cp_plane{base->GetWritePtr(p) + offset, base->GetPitch(p), step}, overlap.base_x >> ws,
                          overlap.base_y >> hs);
    cp_const_plane src = Offset(cp_const_plane{source->GetReadPtr(p) + offset, source->GetPitch(p), step},
                                overlap.source_x >> ws, overlap.source_y >> hs);
    cp_sampling sampling{source_vi.width, source_vi.height, 1 << ws,         1 << hs,
                         placement,       overlap.source_x, overlap.source_y};
    ...
    if (select) {
      ...
      sampling.source_width = vi.width;
      sampling.source_height = vi.height;
      sampling.origin_x = overlap.base_x;
      sampling.origin_y = overlap.base_y;
    ...
    if (c != 0) {
      ...
      if (!chroma) {
        ...
        if (operation == LayerOperation::Multiply)
          current.opacity *= .5;
      } else if (operation == LayerOperation::Multiply)
        current.operation = CP_MIX;
    }
    ...
    Check(k->process_plane(&current, Read(dst), src, alpha && !fast && !select ? &m : nullptr, select ? &b : nullptr,
                           select ? &s : nullptr, dst, part),
          env);
  }
}
} // namespace aif::filters::layer
