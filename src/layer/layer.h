...
{
public:
  Layer(PClip _child1, PClip _child2, const char _op[], int _lev, int _x, int _y, int _t, bool _chroma, float _strength,
        int _placement, IScriptEnvironment* env);
  ...
  inline virtual const VideoInfo& __stdcall GetVideoInfo() override { return vi; }
  inline virtual bool __stdcall GetParity(int n) override { return child1->GetParity(n); }
...
private:
  const cp_kernels* kernels_ = nullptr;
