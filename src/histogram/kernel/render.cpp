...
PVideoFrame Histogram::DrawModeAudioLevels(int n, IScriptEnvironment* env) {
  ...
  for (int i = 0; i < lines; i++) {
    ...
    if (!(i & 1)) {
      ...
      DrawStringPlanar(vi, src, 0, i ? lines_y[i] : TEXT_HEIGHT / 2, text, env);
    }
  }
  ...
  for (int ch = 0; ch < channels; ch++) {
    ...
    DrawStringPlanar(vi, src, ((ch * 2) + 1) * bar_w, vi.height - 2 * TEXT_HEIGHT + TEXT_HEIGHT / 2, text, env);
    snprintf(text, sizeof(text), "%6.2fdB", (float)-ch_rms);
    DrawStringPlanar(vi, src, ((ch * 2) + 1) * bar_w, vi.height - 1 * TEXT_HEIGHT + TEXT_HEIGHT / 2, text, env);
  }
...
PVideoFrame Histogram::DrawModeOverlay(int n, IScriptEnvironment* env) {
  ...
  initialize_alpha(dst, src, vi, keepsource || mode == ModeOverlay, env);
...
PVideoFrame Histogram::DrawModeLuma(int n, IScriptEnvironment* env) {
  ...
  if (vi.IsYUY2()) {
    ...
    if (bits_per_pixel == 8) {
      ...
      const int max_pixel_value = bits_per_pixel == 32 ? 0 : (1 << bits_per_pixel) - 1;
      for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
          const int pixel = reinterpret_cast<uint16_t*>(srcp)[x] << 4;
          reinterpret_cast<uint16_t*>(srcp)[x] =
              (uint16_t)((pixel & overlimit) ? (max_pixel_value - (pixel & max_pixel_value)) : pixel & max_pixel_value);
        }
        srcp += pitch;
      }
    } else {
...
void DrawModeColor2_erase_area(int bits_per_pixel, uint8_t* dstp, uint8_t* dstp_u, uint8_t* dstp_v, int pitch,
                               int pitchUV, int height, int heightUV) {
  pixel_t black;
...
template <typename pixel_t>
void DrawModeColor2_draw_misc(int bits_per_pixel, uint8_t* dstp8, uint8_t* dstp8_u, uint8_t* dstp8_v, int pitch,
                              int pitchUV, int height, int heightUV, int show_bits, int swidth, int sheight,
                              double innerF) {
  pixel_t black;
  pixel_t middle_chroma;
  pixel_t luma128;
  ...
  float LC_f[21] = {c8tof(145),  uv8tof(54),  uv8tof(34), c8tof(145),  uv8tof(54),  uv8tof(34), c8tof(210),
                    uv8tof(16),  uv8tof(146), c8tof(210), uv8tof(16),  uv8tof(146), c8tof(81),  uv8tof(90),
                    uv8tof(240), c8tof(81),   uv8tof(90), uv8tof(240), c8tof(81),   uv8tof(90), uv8tof(240)};
  // cyan, 4x blue, magenta, red:
  float RC_f[21] = {c8tof(170),  uv8tof(166), uv8tof(16),  c8tof(41),   uv8tof(240), uv8tof(110), c8tof(41),
                    uv8tof(240), uv8tof(110), c8tof(41),   uv8tof(240), uv8tof(110), c8tof(41),   uv8tof(240),
                    uv8tof(110), c8tof(106),  uv8tof(202), uv8tof(222), c8tof(81),   uv8tof(90),  uv8tof(240)};
    ...
    for (int x = -limit; x <= 0; x++) {
      ...
      if (distSq <= outerSq && distSq >= innerSq) {
        ...
        if constexpr (std::is_integral<pixel_t>::value) {
          ...
          dstp[xP + yP * pitch] = (pixel_t)((interp * (LC[3 * activeY] << factorshift)) >> 8); // left upper half
          ...
          p_uv = xP + yP * pitchUV;
          dstp_u[p_uv] =
              (pixel_t)((dstp_u[p_uv] * invInt + interp * (LC[3 * activeY + 1] << factorshift)) >> 8); // left half
          dstp_v[p_uv] =
              (pixel_t)((dstp_v[p_uv] * invInt + interp * (LC[3 * activeY + 2] << factorshift)) >> 8); // left half

          xP = ((limit_showwidth) >> swidth) - xP;
          p_uv = xP + yP * pitchUV;

          dstp_u[p_uv] =
              (pixel_t)((dstp_u[p_uv] * invInt + interp * (RC[3 * activeY + 1] << factorshift)) >> 8); // right half
          dstp_v[p_uv] =
              (pixel_t)((dstp_v[p_uv] * invInt + interp * (RC[3 * activeY + 2] << factorshift)) >> 8); // right half
        } else {
          // 32 bit float
          ...
          dstp[xP + yP * pitch] = (pixel_t)(interp * LC_f[3 * activeY]); // left upper half

          dstp[limit_showwidth - xP + yP * pitch] = (pixel_t)(interp * RC_f[3 * activeY]); // right upper half
...
template <typename pixel_t>
void do_vectorscope_color2(pixel_t* pdstb, pixel_t* pdstbU, pixel_t* pdstbV, const pixel_t* pY, const pixel_t* pU,
                           const pixel_t* pV, int dst_pitch, int dst_pitchUV, int src_pitch, int src_pitchUV,
                           int src_widthUV, int src_heightUV, int swidth, int sheight, int shift, int bits_per_pixel) {
  if constexpr (sizeof(pixel_t) == 1 || sizeof(pixel_t) == 2) {
    ...
    for (int y = 0; y < src_heightUV; y++) {
      for (int x = 0; x < src_widthUV; x++) {
        ...
        const int uval = std::min(65535, (int)((uval_f * 65536 + 32768) + 0.5f)); // simulate on 16 bits
        const int vval = std::min(65535, (int)((vval_f * 65536 + 32768) + 0.5f));
...
PVideoFrame Histogram::DrawModeColor2(int n, IScriptEnvironment* env) {
  ...
  if (keepsource) {
    if (src->GetHeight() < dst->GetHeight()) {
      if (bits_per_pixel == 8)
        DrawModeColor2_erase_area<uint8_t>(bits_per_pixel, pdst, pdstU, pdstV, dst_pitch, dst_pitchUV, dst_height,
                                           dst_heightUV);
      else if (bits_per_pixel <= 16)
        DrawModeColor2_erase_area<uint16_t>(bits_per_pixel, pdst, pdstU, pdstV, dst_pitch, dst_pitchUV, dst_height,
                                            dst_heightUV);
      else
        DrawModeColor2_erase_area<float>(bits_per_pixel, pdst, pdstU, pdstV, dst_pitch, dst_pitchUV, dst_height,
                                         dst_heightUV);
    }
  }
  ...
  if (keepsource) {
    env->BitBlt(pdstU, dst_pitchUV, src->GetReadPtr(PLANAR_U), src->GetPitch(PLANAR_U), src->GetRowSize(PLANAR_U),
                src->GetHeight(PLANAR_U));
    env->BitBlt(pdstV, dst_pitchUV, src->GetReadPtr(PLANAR_V), src->GetPitch(PLANAR_V), src->GetRowSize(PLANAR_V),
                src->GetHeight(PLANAR_V));
  }
  ...
  if (bits_per_pixel == 8)
    DrawModeColor2_draw_misc<uint8_t>(bits_per_pixel, pdstb, pdstbU, pdstbV, dst_pitch, dst_pitchUV, dst_height,
                                      dst_heightUV, show_bits, swidth, sheight, color2_innerF);
  else if (bits_per_pixel <= 16)
    DrawModeColor2_draw_misc<uint16_t>(bits_per_pixel, pdstb, pdstbU, pdstbV, dst_pitch, dst_pitchUV, dst_height,
                                       dst_heightUV, show_bits, swidth, sheight, color2_innerF);
  else
    DrawModeColor2_draw_misc<float>(bits_per_pixel, pdstb, pdstbU, pdstbV, dst_pitch, dst_pitchUV, dst_height,
                                    dst_heightUV, show_bits, swidth, sheight, color2_innerF);
  ...
  for (int i = 0; i < 24; i++) {
    if (pixelsize == 1)
      ...
      ((uint16_t*)pdstb)[deg15c[i] + deg15s[i] * dst_pitch / sizeof(uint16_t)] =
          235 << (bits_per_pixel - 8);                                                 // 235: Y-maxluma
    else                                                                               // if (pixelsize == 4)
  ...
  if (bits_per_pixel == 8)
    do_vectorscope_color2<uint8_t>(pdstb, pdstbU, pdstbV, pY, pU, pV, dst_pitch, dst_pitchUV, src_pitch, src_pitchUV,
                                   src_widthUV, src_heightUV, swidth, sheight, shift, bits_per_pixel);
  else if (bits_per_pixel <= 16)
    do_vectorscope_color2<uint16_t>((uint16_t*)pdstb, (uint16_t*)pdstbU, (uint16_t*)pdstbV, (uint16_t*)pY,
                                    (uint16_t*)pU, (uint16_t*)pV, dst_pitch, dst_pitchUV, src_pitch, src_pitchUV,
                                    src_widthUV, src_heightUV, swidth, sheight, shift, bits_per_pixel);
  else
    do_vectorscope_color2<float>((float*)pdstb, (float*)pdstbU, (float*)pdstbV, (float*)pY, (float*)pU, (float*)pV,
                                 dst_pitch, dst_pitchUV, src_pitch, src_pitchUV, src_widthUV, src_heightUV, swidth,
                                 sheight, shift, bits_per_pixel);
...
PVideoFrame Histogram::DrawModeLevels(int n, IScriptEnvironment* env) {
  ...
  int plane_default_black[3] = {RGB ? 0 : (16 << color_shift), RGB ? 0 : (128 << color_shift),
                                RGB ? 0 : (128 << color_shift)};
  float plane_default_black_f[3] = {RGB ? 0 : 16 / 255.0f, RGB ? 0 : 0.0f, RGB ? 0 : 0.0f};
  ...
  if (keepsource) {
    if (src->GetHeight() < dst->GetHeight()) {
      ...
      switch (pixelsize) {
        case 1:
          memset(dstp + fillStart, plane_default_black[0], fillSize);
          break;
        case 2:
          std::fill_n((uint16_t*)(dstp + fillStart), fillSize / sizeof(uint16_t), plane_default_black[0]);
          break;
        case 4:
          std::fill_n((float*)(dstp + fillStart), fillSize / sizeof(float), plane_default_black_f[0]);
          break;
      }
      ...
      for (int p = 1; p < planecount; p++) {
        ...
        switch (pixelsize) {
          case 1:
            memset(ptr + fillStart, RGB ? 0 : plane_default_black[p], fillSize);
            break;
          case 2:
            std::fill_n((uint16_t*)(ptr + fillStart), fillSize / sizeof(uint16_t), plane_default_black[p]);
            break;
          case 4:
            std::fill_n((float*)(ptr + fillStart), fillSize / sizeof(float), plane_default_black_f[p]);
            break;
        }
      }
    }
  ...
  if (!histPlane1)
    env->ThrowError("Histogram: Could not reserve memory.");
  uint32_t* histPlanes[3] = {histPlane1, planecount == 1 ? nullptr : histPlane1 + show_size,
  ...
  if (vi.IsPlanar()) {
    ...
    for (int p = 0; p < planecount; p++) {
      ...
      get_bits_conv_constants(d, chroma, full_range, full_range, bits_per_pixel /*source_bitdepth*/,
                              show_bits /*target_bitdepth*/);
        ...
        if (show_bits == bits_per_pixel) {
          ...
          if (full_range) {
            ...
            if (chroma) {
              ...
              for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                  hist[clamp((int)((srcp8[x] - d.src_offset_i) * d.mul_factor + dst_offset_plus_round), 0,
                             max_show_pixel_value)]++;
                }
                srcp8 += pitch;
              }
            } else {
        ...
        if (show_bits == bits_per_pixel) {
          ...
          if (full_range) {
            ...
            if (chroma) {
              ...
              for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                  hist[clamp((int)((srcp16[x] - d.src_offset_i) * d.mul_factor + dst_offset_plus_round), 0,
                             max_show_pixel_value)]++;
                }
                srcp16 += pitch;
              }
    ...
    for (int p = 0; p < planecount; p++) {
      ...
      for (int y = 0; y < dst->GetHeight() >> sheight; y++) {
        switch (pixelsize) {
          case 1:
            memset(ptr, plane_default_black[p], show_size >> swidth);
            break;
          case 2:
            std::fill_n((uint16_t*)(ptr), show_size >> swidth, plane_default_black[p]);
            break;
          case 4:
            std::fill_n((float*)(ptr), show_size >> swidth, plane_default_black_f[p]);
            break;
        }
        ptr += dstPitch;
      }
        ...
        if (RGB) {
          ...
          if (!isGrey) {
            ...
            for (int gradient_upper_lower = 0; gradient_upper_lower < 2; gradient_upper_lower++) {
              ...
              const int color1_upper_lower_gradient[2][3] = {{210 / 2, 16 + 112 / 2, 128},
                                                             {170 / 2, 128, 16 + 112 / 2}};
              const int color2_upper_lower_gradient[2][3] = {{41 / 2, 240 - 112 / 2, 128},
                                                             {81 / 2, 128, 240 - 112 / 2}};
              float color1_upper_lower_gradient_f[2][3] = {{c8tof(210 / 2), uv8tof(16 + 112 / 2), uv8tof(128)},
                                                           {c8tof(170 / 2), uv8tof(128), uv8tof(16 + 112 / 2)}};
              float color2_upper_lower_gradient_f[2][3] = {{c8tof(41 / 2), uv8tof(240 - 112 / 2), uv8tof(128)},
                                                           {c8tof(81 / 2), uv8tof(128), uv8tof(240 - 112 / 2)}};
                ...
                if (plane == PLANAR_Y) {
                  ...
                  if (pixelsize == 1) {
                    for (; x <= show_middle_pos; x++) {
                      int color3 = (gradient_upper_lower == 0) ? (((show_middle_pos - x) * 15) >> 3) >> pos_shift
                                                               :                           // *1.875
                ...
                if (plane == PLANAR_Y) {
                  if (pixelsize == 1) {
                    for (; x <= (240 << pos_shift) >> swidth; x++) {
                      int color4 = (gradient_upper_lower == 0) ? ((x - show_middle_pos) * 24001) >> 16 >> pos_shift
                                                               :                           // *0.366
    ...
    for (int p = 0; p < (RGB ? 3 : 1); p++) {
      ...
      unsigned char* pdstb = dst->GetWritePtr(plane);
      pdstb +=
          (xstart * pixelsize); // next to the original clip (if kept), working only on Y plane: no ">> swidth" needed
      BYTE* ptr = pdstb;
      ...
      if (markers) {
        ...
        for (int y = 0; y <= 64 + (planecount - 1) * (16 + 64); y++) {
          if ((y & 3) > 1) {
            if (pixelsize == 1)
              ptr[show_middle_pos] = color_i;
            else if (pixelsize == 2)
              reinterpret_cast<uint16_t*>(ptr)[show_middle_pos] = color_i;
            else
              reinterpret_cast<float*>(ptr)[show_middle_pos] = color_f;
          }
          ptr += dstPitch;
        }
      }
    ...
    for (int p = 0; p < (RGB ? 3 : 1); p++) {
      ...
      for (int n = 0; n < planecount; n++) {
        ...
        const uint32_t clampval =
            (int)((src_width * src_height) * option.AsDblDef(100.0) / 100.0); // Population limit % factor
        uint32_t maxval = 0;
        uint32_t* hist;
        ...
        switch (n) { // n: YUV 012, GBR 012
          case 0:
            Y_pos = RGB ? 128 + 16 : 64 + 0;
            break; // Y or G
          case 1:
            Y_pos = RGB ? 192 + 32 : 128 + 16;
            break; // U or B
          case 2:
            Y_pos = RGB ? 64 + 0 : 192 + 32;
            break; // V or R
        }
...
PVideoFrame Histogram::DrawModeClassic(int n, IScriptEnvironment* env) {
  ...
  int hist_mid_range_luma =
      (hist_range_luma + 1) / 2 + hist_tv_range_low - 1; // in Classic Avisynth somehow 124 was fixed for this
      ...
      if (markers) {
        if (pixelsize == 1) {
          ...
          for (int x = 0; x < show_size; ++x) {
            ...
            if (x < hist_tv_range_low || x == hist_mid_range_luma || x > hist_tv_range_hi_luma) {
              dstp32[x] =
                  (exptab[min(E167, h)] + (68 << (internal_bits_per_pixel - 8))) / 65535.0f; // fake 0..65535 to 0..1.0
            } else {
