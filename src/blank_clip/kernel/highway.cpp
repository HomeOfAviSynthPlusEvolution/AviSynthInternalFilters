...
extern "C" uint32_t aif_blank_clip_supported_cpu(void) {
#ifndef AIF_SCALAR_ONLY
  const auto t = hwy::SupportedTargets();
  uint32_t cpu = 0;
#if HWY_TARGETS & HWY_SSE2
  if (t & HWY_SSE2)
    cpu |= AIF_BLANK_CLIP_SSE2;
#endif
#if HWY_TARGETS & HWY_SSSE3
  if (t & HWY_SSSE3)
    cpu |= AIF_BLANK_CLIP_SSSE3;
#endif
...
aif::blank_clip::Fill aif::blank_clip::backend(uint32_t cpu) {
  cpu &= aif_blank_clip_supported_cpu();
#ifndef AIF_SCALAR_ONLY
#if HWY_TARGETS & HWY_AVX10_2
  if (cpu & AIF_BLANK_CLIP_AVX10_2)
    return HWY_CHOOSE_AVX10_2(FillRow);
#endif
#if HWY_TARGETS & HWY_AVX3_SPR
  if (cpu & AIF_BLANK_CLIP_AVX3_SPR)
    return HWY_CHOOSE_AVX3_SPR(FillRow);
#endif
#if HWY_TARGETS & HWY_AVX3_ZEN4
  if (cpu & AIF_BLANK_CLIP_AVX3_ZEN4)
    return HWY_CHOOSE_AVX3_ZEN4(FillRow);
#endif
#if HWY_TARGETS & HWY_AVX3_DL
  if (cpu & AIF_BLANK_CLIP_AVX3_DL)
    return HWY_CHOOSE_AVX3_DL(FillRow);
#endif
#if HWY_TARGETS & HWY_AVX3
  if (cpu & AIF_BLANK_CLIP_AVX3)
    return HWY_CHOOSE_AVX3(FillRow);
#endif
#if HWY_TARGETS & HWY_AVX2
  if (cpu & AIF_BLANK_CLIP_AVX2)
    return HWY_CHOOSE_AVX2(FillRow);
#endif
#if HWY_TARGETS & HWY_SSE4
  if (cpu & AIF_BLANK_CLIP_SSE4)
    return HWY_CHOOSE_SSE4(FillRow);
#endif
#if HWY_TARGETS & HWY_SSSE3
  if (cpu & AIF_BLANK_CLIP_SSSE3)
    return HWY_CHOOSE_SSSE3(FillRow);
#endif
#if HWY_TARGETS & HWY_SSE2
  if (cpu & AIF_BLANK_CLIP_SSE2)
    return HWY_CHOOSE_SSE2(FillRow);
#endif
#if HWY_TARGETS & HWY_NEON_WITHOUT_AES
  if (cpu & AIF_BLANK_CLIP_NEON)
