...
FixBrokenChromaUpsampling::FixBrokenChromaUpsampling(PClip _clip, IScriptEnvironment* env) : GenericVideoFilter(_clip) {
  if (!vi.IsYUY2())
...
PVideoFrame __stdcall FixBrokenChromaUpsampling::GetFrame(int n, IScriptEnvironment* env) {
  ...
  swap_chroma(frame->GetWritePtr(), frame->GetPitch(), frame->GetRowSize(), frame->GetHeight());
  return frame;
}
AVSValue __cdecl FixBrokenChromaUpsampling::Create(AVSValue args, void*, IScriptEnvironment* env) {
