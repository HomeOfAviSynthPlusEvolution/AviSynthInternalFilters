...
PVideoFrame FixLuminance::GetFrame(int n, IScriptEnvironment* env) {
  ...
  darken(frame->GetWritePtr(), frame->GetPitch(), vi.width, vi.height, vertex, slope);
  return frame;
}
AVSValue __cdecl FixLuminance::Create(AVSValue args, void*, IScriptEnvironment* env) {
  const double value = args[2].AsFloat() * 16;
  if (!std::isfinite(value) || value < INT32_MIN || value > INT32_MAX)
    env->ThrowError("FixLuminance: slope is outside integer range.");
  return new FixLuminance(args[0].AsClip(), args[1].AsInt(), int(value), env);
}
} // namespace aif::filters::legacy_correction
