...
void ValidateFPS(const char* name, uint64_t num, uint64_t den, IScriptEnvironment* env) {
  if (!num || !den)
    env->ThrowError("%s: FPS numerator and denominator must be positive.", name);
}
void FrameRatio(const char* name, unsigned sn, unsigned sd, unsigned dn, unsigned dd, int64_t& a, int64_t& b,
                IScriptEnvironment* env) {
  ValidateFPS(name, sn, sd, env);
  ValidateFPS(name, dn, dd, env);
  uint64_t x = uint64_t(sn) * dd, y = uint64_t(sd) * dn;
  const uint64_t divisor = std::gcd(x, y);
  x /= divisor;
  y /= divisor;
  // For every valid signed 32-bit frame index/count, both n*a and
  // count*b + a/2 fit int64_t when the reduced factors fit uint32_t.
  if (x > UINT32_MAX || y > UINT32_MAX)
    env->ThrowError("%s: Reduced FPS conversion factors exceed 4294967295; 64-bit frame arithmetic would not be safe "
                    "for all frame counts.",
                    name);
  a = int64_t(x);
  b = int64_t(y);
}
int ScaleAudioRate(const char* name, int rate, uint64_t num, uint64_t den, IScriptEnvironment* env) {
  if (!rate)
    return 0; // No audio.
  const uint64_t divisor = std::gcd(num, den);
  num /= divisor;
  den /= divisor;
  if (rate < 0 || !den || num > (uint64_t(INT64_MAX) - den / 2) / uint64_t(rate))
    env->ThrowError("%s: Synchronized audio sample rate calculation overflows.", name);
  const uint64_t result = (uint64_t(rate) * num + den / 2) / den;
  if (!result || result > INT32_MAX)
    env->ThrowError("%s: Synchronized audio sample rate must fit a positive 32-bit integer.", name);
  return int(result);
}
...
static bool float_to_frac(float input, unsigned& num, unsigned& den) {
  ...
  mantissa = (value & 0x7FFFFF) + 0x800000;           // add implicit bit on the left
  exponent = ((value & 0x7F800000) >> 23) - 127 - 23; // remove decimal pt
...
static bool reduce_float(float value, unsigned& num, unsigned& den) {
  ...
  for (;;) // calculate convergents
  {
    ...
    if (f2 == 0)
      break; // no more convergents (n2 / d2 == input)
...
AVSValue __cdecl ContinuedCreate(AVSValue args, void* key, IScriptEnvironment* env) {
  ...
  if (args[1].IsInt()) { // num, den[, limit] form
    if (args[0].IsInt()) {
      ...
      const double value = args[0].AsFloat();
      if (!std::isfinite(value) || value < 0 || value > 4294967295.0)
        env->ThrowError("ContinuedFraction: Numerator out of range.");
      num = (uint32_t)value;
...
void FloatToFPS(const char* name, float n, uint32_t& num, uint32_t& den, IScriptEnvironment* env) {
  ...
  float x;
  uint32_t u = 0;
  if (double(n) * 1001 + 0.5 <= 4294967295.0) {
    u = (uint32_t)(double(n) * 1001 + 0.5);
  ...
  if (n < 14.986 && 30000.0 / n + 0.5 <= 4294967295.0) {
...
void PresetToFPS(const char* name, const char* p, uint32_t& num, uint32_t& den, IScriptEnvironment* env) {
  if (lstrcmpi(p, "ntsc_film") == 0) {
    num = 24000;
    den = 1001;
  } else if (lstrcmpi(p, "ntsc_video") == 0) {
    num = 30000;
    den = 1001;
  } else if (lstrcmpi(p, "ntsc_double") == 0) {
    num = 60000;
    den = 1001;
  } else if (lstrcmpi(p, "ntsc_quad") == 0) {
    num = 120000;
    den = 1001;
  }

  else if (lstrcmpi(p, "ntsc_round_film") == 0) {
    num = 2997;
    den = 125;
  } else if (lstrcmpi(p, "ntsc_round_video") == 0) {
    num = 2997;
    den = 100;
  } else if (lstrcmpi(p, "ntsc_round_double") == 0) {
    num = 2997;
    den = 50;
  } else if (lstrcmpi(p, "ntsc_round_quad") == 0) {
    num = 2997;
    den = 25;
  }

  else if (lstrcmpi(p, "film") == 0) {
    num = 24;
    den = 1;
  }

  else if (lstrcmpi(p, "pal_film") == 0) {
    num = 25;
    den = 1;
  } else if (lstrcmpi(p, "pal_video") == 0) {
    num = 25;
    den = 1;
  } else if (lstrcmpi(p, "pal_double") == 0) {
    num = 50;
    den = 1;
  } else if (lstrcmpi(p, "pal_quad") == 0) {
    num = 100;
    den = 1;
  }

  else if (lstrcmpi(p, "drop24") == 0) {
    num = 24000;
    den = 1001;
  } else if (lstrcmpi(p, "drop30") == 0) {
    num = 30000;
    den = 1001;
  } else if (lstrcmpi(p, "drop60") == 0) {
    num = 60000;
    den = 1001;
  } else if (lstrcmpi(p, "drop120") == 0) {
    num = 120000;
    den = 1001;
  }
  /*
