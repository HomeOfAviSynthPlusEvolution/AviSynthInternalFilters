...
static int getPixelTypeWithoutAlpha(VideoInfo& vi) {
  return (vi.IsYUVA() ? (vi.pixel_type & ~VideoInfo::CS_YUVA) | VideoInfo::CS_YUV
                      : (vi.IsPlanarRGBA() ? (vi.pixel_type & ~VideoInfo::CS_RGBA_TYPE) | VideoInfo::CS_RGB_TYPE
                                           : vi.pixel_type));
}

Overlay::Overlay(PClip _child, AVSValue args, IScriptEnvironment* env)
    : GenericVideoFilter(_child), kernels_(select_kernels(env)),
      chroma_(select_chroma(allowed_targets(aif::filters::host_cpu_flags(env)))) {
  ...
  opacity = (int)std::clamp(256.0 * opacity_f + 0.5, double(INT_MIN),
                            double(INT_MAX)); // range is converted to 256 for all all bit_depth
  ...
  if (output_pixel_format_override) {
    int output_pixel_type = pixel_type_from_name(output_pixel_format_override);
  ...
  if (vi.IsRGB() && (_stricmp(name, "Luma") == 0 || _stricmp(name, "Chroma") == 0)) {
    ...
    use444 = true;
  } else if (!use444_defined && (vi.IsY() || vi.Is420() || vi.Is422() || vi.IsRGB()) &&
  ...
  if (use444) {
    ...
    switch (bits_per_pixel) {
      case 8:
        viInternalWorkingFormat.pixel_type = hasAlpha ? VideoInfo::CS_YUVA444 : VideoInfo::CS_YV24;
        break;
      case 10:
        viInternalWorkingFormat.pixel_type = hasAlpha ? VideoInfo::CS_YUVA444P10 : VideoInfo::CS_YUV444P10;
        break;
      case 12:
        viInternalWorkingFormat.pixel_type = hasAlpha ? VideoInfo::CS_YUVA444P12 : VideoInfo::CS_YUV444P12;
        break;
      case 14:
        viInternalWorkingFormat.pixel_type = hasAlpha ? VideoInfo::CS_YUVA444P14 : VideoInfo::CS_YUV444P14;
        break;
      case 16:
        viInternalWorkingFormat.pixel_type = hasAlpha ? VideoInfo::CS_YUVA444P16 : VideoInfo::CS_YUV444P16;
        break;
      case 32:
        viInternalWorkingFormat.pixel_type = hasAlpha ? VideoInfo::CS_YUVA444PS : VideoInfo::CS_YUV444PS;
        break;
    }
  } else {
  ...
  if (outputVi.Is420() && use444) {
    ...
    switch (bits_per_pixel) {
      case 8:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA420 : VideoInfo::CS_YV12;
        break;
      case 10:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA420P10 : VideoInfo::CS_YUV420P10;
        break;
      case 12:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA420P12 : VideoInfo::CS_YUV420P12;
        break;
      case 14:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA420P14 : VideoInfo::CS_YUV420P14;
        break;
      case 16:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA420P16 : VideoInfo::CS_YUV420P16;
        break;
      case 32:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA420PS : VideoInfo::CS_YUV420PS;
        break;
    }
    ...
    switch (bits_per_pixel) {
      case 8:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA422 : VideoInfo::CS_YV16;
        break;
      case 10:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA422P10 : VideoInfo::CS_YUV422P10;
        break;
      case 12:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA422P12 : VideoInfo::CS_YUV422P12;
        break;
      case 14:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA422P14 : VideoInfo::CS_YUV422P14;
        break;
      case 16:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA422P16 : VideoInfo::CS_YUV422P16;
        break;
      case 32:
        vi.pixel_type = hasAlpha ? VideoInfo::CS_YUVA422PS : VideoInfo::CS_YUV422PS;
        break;
    }
...
PVideoFrame __stdcall Overlay::GetFrame(int n, IScriptEnvironment* env) {
  ...
  if (inputVi.pixel_type == viInternalWorkingFormat.pixel_type) {
    ...
    if (inputVi.Is420()) {
      ...
      Convert444FromYV12(Inframe, frame, pixelsize, bits_per_pixel, env, chroma_);
    } else if (inputVi.Is422()) {
      // use blazing fast YV16 -> YV24 converter
      PVideoFrame Inframe = child->GetFrame(n, env);
      frame = env->NewVideoFrameP(viInternalWorkingFormat, &Inframe);
      Convert444FromYV16(Inframe, frame, pixelsize, bits_per_pixel, env, chroma_);
  ...
  ImageOverlayInternal* img = new ImageOverlayInternal(
      frame, vi.width, vi.height, viInternalWorkingFormat,
      child->GetVideoInfo().IsYUVA() || child->GetVideoInfo().IsPlanarRGBA(), false, env, chroma_);
  ...
  if (overlayVi.pixel_type == viInternalWorkingFormat.pixel_type) {
    ...
    if (of_mode == OF_Multiply) {
      ...
      // no fancy options for chroma resampler, etc.. simply fast
      Convert444FromYV12(frame, Oframe, pixelsize, bits_per_pixel, env, chroma_);
    } else if (overlayVi.Is422()) {
      // use blazing fast YV16 -> YV24 converter
      PVideoFrame frame = overlay->GetFrame(n, env);
      Oframe = env->NewVideoFrameP(viInternalOverlayWorkingFormat, &frame);
      Convert444FromYV16(frame, Oframe, pixelsize, bits_per_pixel, env, chroma_);
    } else {
      if (!overlayVi.Is444())
        env->ThrowError(
            "Overlay: internal error, wrong internal overlayVi format for internal 444, must be 420,422 or 444");
      Oframe = overlay->GetFrame(n, env);
    }
    ...
    if (overlayVi.Is420()) {
      ...
      Oframe = env->NewVideoFrameP(viInternalOverlayWorkingFormat, &frame);
      // no fancy options for chroma resampler, etc.. simply fast
      Convert444ToYV12(frame, Oframe, pixelsize, bits_per_pixel, env, chroma_);
    } else {
    ...
    if (overlayVi.Is422()) {
      ...
      Oframe = env->NewVideoFrameP(viInternalOverlayWorkingFormat, &frame);
      // no fancy options for chroma resampler, etc.. simply fast
      Convert444ToYV16(frame, Oframe, pixelsize, bits_per_pixel, env, chroma_);
    } else {
  ...
  if (of_mode == OF_Multiply) {
    ...
    switch (bits_per_pixel) {
      case 8:
        actual_viInternalOverlayWorkingFormat.pixel_type = VideoInfo::CS_Y8;
        break;
      case 10:
        actual_viInternalOverlayWorkingFormat.pixel_type = VideoInfo::CS_Y10;
        break;
      case 12:
        actual_viInternalOverlayWorkingFormat.pixel_type = VideoInfo::CS_Y12;
        break;
      case 14:
        actual_viInternalOverlayWorkingFormat.pixel_type = VideoInfo::CS_Y14;
        break;
      case 16:
        actual_viInternalOverlayWorkingFormat.pixel_type = VideoInfo::CS_Y16;
        break;
      case 32:
        actual_viInternalOverlayWorkingFormat.pixel_type = VideoInfo::CS_Y32;
        break;
    }
  }

  ImageOverlayInternal* overlayImg = new ImageOverlayInternal(
      Oframe, overlayVi.width, overlayVi.height, actual_viInternalOverlayWorkingFormat,
      overlay->GetVideoInfo().IsYUVA() || overlay->GetVideoInfo().IsPlanarRGBA(), false, env, chroma_);

  // Clip overlay to original image
  ClipFrames(img, overlayImg, int64_t(offset_x) + con_x_offset, int64_t(offset_y) + con_y_offset);
  ...
  if (overlayImg->IsSizeZero()) { // Nothing to overlay
    ...
    if (mask) {
      ...
      if (greymask || getPixelTypeWithoutAlpha(maskVi) == getPixelTypeWithoutAlpha(viInternalWorkingFormat) ||
        ...
        if (maskVi.Is420()) {
          ...
          // no fancy options for chroma resampler, etc.. simply fast
          Convert444FromYV12(frame, Mframe, pixelsize, bits_per_pixel, env, chroma_);
          // convert frameSrc420 -> frame
        } else if (maskVi.Is422()) {
          // use blazing fast YV16 -> YV24 converter
          PVideoFrame frame = mask->GetFrame(n, env);
          Mframe = env->NewVideoFrameP(viInternalOverlayWorkingFormat, &frame);
          Convert444FromYV16(frame, Mframe, pixelsize, bits_per_pixel, env, chroma_);
        } else
        ...
        if (isInternal420) {
          ...
          Mframe = env->NewVideoFrameP(viInternalOverlayWorkingFormat, &frame);
          // no fancy options for chroma resampler, etc.. simply fast
          Convert444ToYV12(frame, Mframe, pixelsize, bits_per_pixel, env, chroma_);
        } else if (isInternal422) {
          // mask is 444 here
          PVideoFrame frame = mask->GetFrame(n, env);
          Mframe = env->NewVideoFrameP(viInternalOverlayWorkingFormat, &frame);
          // no fancy options for chroma resampler, etc.. simply fast
          Convert444ToYV16(frame, Mframe, pixelsize, bits_per_pixel, env, chroma_);
        } else {
          if (!maskVi.Is444())
      ...
      maskImg = new ImageOverlayInternal(Mframe, maskVi.width, maskVi.height, viInternalOverlayWorkingFormat,
                                         mask->GetVideoInfo().IsYUVA() || mask->GetVideoInfo().IsPlanarRGBA(), greymask,
                                         env, chroma_);

      img->ReturnOriginal(true);
      ClipFrames(img, maskImg, int64_t(offset_x) + con_x_offset, int64_t(offset_y) + con_y_offset);
    }
    ...
    func->setOpacity(int(std::clamp<int64_t>(int64_t(opacity) + op_offset, 0, 256)), opacity_f + op_offset_f);
  ...
  if (outputVi.Is420() && viInternalWorkingFormat.Is444()) {
    ...
    Convert444ToYV12(frame, outputFrame, pixelsize, bits_per_pixel, env, chroma_);
    return outputFrame;
  } else if (outputVi.Is422() && viInternalWorkingFormat.Is444()) {
    PVideoFrame outputFrame = env->NewVideoFrameP(outputVi, &frame);
    Convert444ToYV16(frame, outputFrame, pixelsize, bits_per_pixel, env, chroma_);
    return outputFrame;
  }
...
OverlayFunction* Overlay::SelectFunction() {
  return CreateCompositeOverlay(kernels_);
}

void Overlay::ClipFrames(ImageOverlayInternal* input, ImageOverlayInternal* overlay, int64_t x, int64_t y) {
  input->ResetFake();
  overlay->ResetFake();
  input->ReturnOriginal(false);
  overlay->ReturnOriginal(false);
  const int64_t left = std::max<int64_t>(0, x), top = std::max<int64_t>(0, y);
  const int64_t right = std::min<int64_t>(input->w(), x + overlay->w());
  const int64_t bottom = std::min<int64_t>(input->h(), y + overlay->h());
  if (right <= left || bottom <= top) {
    input->SubFrame(0, 0, 0, 0);
    overlay->SubFrame(0, 0, 0, 0);
    return;
  }
  const int width = int(right - left), height = int(bottom - top);
  input->SubFrame(int(left), int(top), width, height);
  overlay->SubFrame(int(left - x), int(top - y), width, height);
}

void Overlay::FetchConditionals(IScriptEnvironment* env, int* op_offset, float* op_offset_f, int* con_x_offset,
                                int* con_y_offset, bool ignore_conditional, const char* condVarSuffix) {
  *op_offset = 0;
  *op_offset_f = 0.0f;
  *con_x_offset = 0;
  *con_y_offset = 0;
  ...
  if (!ignore_conditional) {
    {
      ...
      const double value = env->GetVarDouble(s.c_str(), 0.0);
      if (!std::isfinite(value))
        env->ThrowError("Overlay: conditional opacity must be finite");
      *op_offset = int(std::clamp(value * 256, double(INT_MIN), double(INT_MAX)));
    ...
    }
    {
      ...
      if (!std::isfinite(value) || value < INT_MIN || value > INT_MAX)
        env->ThrowError("Overlay: conditional x must fit a finite integer coordinate");
      *con_x_offset = int(value);
    }
    {
      std::string s = std::string("OL_y_offset") + condVarSuffix;
      const double value = env->GetVarDouble(s.c_str(), 0.0);
      if (!std::isfinite(value) || value < INT_MIN || value > INT_MAX)
        env->ThrowError("Overlay: conditional y must fit a finite integer coordinate");
      *con_y_offset = int(value);
    }
  }
}
