...
#include "kernel/chroma_conversion.h"
...
public:
  ...
  ImageOverlayInternal(PVideoFrame& _frame, int _inw, int _inh, VideoInfo& _workingVI, bool _hasAlpha, bool _grey,
    ...
    if (grey) {
      if (_workingVI.Is420() || _workingVI.Is422()) {
        ...
        maskChroma = static_cast<BYTE*>(Env->Allocate(tmppitch * (_h >> ySubSamplingShifts[1]), 64, AVS_POOLED_ALLOC));
    ...
    if (grey) {
      if (_workingVI.Is420()) {
        ...
        ConvertYToYV12Chroma(maskChroma, origPlanes[0], pitches[1], pitches[0], pixelsize, _w >> xSubSamplingShifts[1],
                             _h >> ySubSamplingShifts[1], Env, chroma);
        origPlanes[1] = origPlanes[2] = maskChroma;
      } else if (_workingVI.Is422()) {
        // resize Y-only mask to 4:2:2 chroma
        ConvertYToYV16Chroma(maskChroma, origPlanes[0], pitches[1], pitches[0], pixelsize, _w >> xSubSamplingShifts[1],
                             _h >> ySubSamplingShifts[1], Env, chroma);
        origPlanes[1] = origPlanes[2] = maskChroma;
      } else {
        // UV = Y plane pointer
  ...
  */
  ...
  void SubFrame(int x, int y, int new_w, int new_h) {
    ...
    for (int p = 0; p < 3; p++) {
      fakePlanes[p] = GetPtrByIndex(p) ? GetPtrByIndex(p) + ptrdiff_t(x >> xSubSamplingShifts[p]) * pixelsize +
                                             ptrdiff_t(y >> ySubSamplingShifts[p]) * pitches[p]
                                       : nullptr;
    }
    fakePlanes[3] = pitches[3] > 0 ? (GetPtrByIndex(3) + ptrdiff_t(x >> xSubSamplingShifts[3]) * pixelsize +
                                      ptrdiff_t(y >> ySubSamplingShifts[3]) * pitches[3])
                                   : nullptr;
  ...
  bool IsSizeZero() {
    ...
    if (h() <= 0)
      return true;
    if (!(pitch && origPlanes[0]))
      return true;
    return false;
  }

  void ReturnOriginal(bool shouldI) { return_original = shouldI; }
