...
class CompositeOverlay final : public OverlayFunction {
  ...
  void Blend(ImageOverlayInternal* base, ImageOverlayInternal* source, ImageOverlayInternal* mask) {
    ...
    for (int c = 0; c < count; ++c) {
      ...
      if (mask)
        masks[c] = {mask->GetPtrByIndex(c), mask->GetPitchByIndex(c), bytes};
    }
    const cp_rows rows{base->w(), base->h(), 0, base->h()};
    if (!rgb && !greyscale && of_mode != OF_Blend && of_mode != OF_Blend_Compat && of_mode != OF_Luma &&
        of_mode != OF_Chroma && of_mode != OF_Lighten && of_mode != OF_Darken) {
      const int operation = of_mode == OF_Add          ? CP_YUV_ADD
                            : of_mode == OF_Subtract   ? CP_YUV_SUBTRACT
                            : of_mode == OF_Multiply   ? CP_YUV_MULTIPLY
                            : of_mode == OF_SoftLight  ? CP_YUV_SOFT_LIGHT
                            : of_mode == OF_HardLight  ? CP_YUV_HARD_LIGHT
                            : of_mode == OF_Difference ? CP_YUV_DIFFERENCE
                                                       : CP_YUV_EXCLUSION;
      ...
      Check(k->process_yuv(&config, {Read(dst[0]), Read(dst[1]), Read(dst[2])}, {src[0], src[1], src[2]},
                           mask ? &m : nullptr, {dst[0], dst[1], dst[2]}, rows),
            env);
      return;
    }
    ...
    config.operation = of_mode == OF_Add        ? CP_ADD
                       : of_mode == OF_Subtract ? CP_SUBTRACT
                       : of_mode == OF_Lighten  ? CP_SELECT_LIGHTER
                       : of_mode == OF_Darken   ? CP_SELECT_DARKER
                                                : CP_MIX;
    config.inclusive = 1;
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if ((of_mode == OF_Luma && c != 0) || (of_mode == OF_Chroma && c == 0))
        continue;
      ...
      if (of_mode == OF_Blend_Compat && bits_per_pixel != 32)
        ...
        Check(k->process_plane(&config, Read(dst[c]), src[c], mask ? &masks[c] : nullptr, guided ? &bg : nullptr,
                               guided ? &src[0] : nullptr, dst[c], part),
              env);
    }
  }

public:
  explicit CompositeOverlay(const cp_kernels* kernels) : kernels_(kernels) {}
...
OverlayFunction* CreateCompositeOverlay(const cp_kernels* kernels) {
  return new CompositeOverlay(kernels);
}
} // namespace aif::filters::overlay
