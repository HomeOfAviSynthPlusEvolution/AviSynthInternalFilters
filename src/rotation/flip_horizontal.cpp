...
PVideoFrame __stdcall FlipHorizontal::GetFrame(int n, IScriptEnvironment* env) {
  ...
  for (int i = 0; i < (vi.IsPlanar() ? vi.NumComponents() : 1); ++i)
    apply(s, d, planes[i], planes[i], pixel_bytes(vi), 3, env);
  return d;
}
AVSValue __cdecl FlipHorizontal::Create(AVSValue args, void*, IScriptEnvironment*) {
