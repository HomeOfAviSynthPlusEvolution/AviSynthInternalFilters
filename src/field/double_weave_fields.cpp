...
DoubleWeaveFields::DoubleWeaveFields(PClip _child, IScriptEnvironment* env) : GenericVideoFilter(_child) {
  if (vi.height > INT32_MAX / 2)
    env->ThrowError("DoubleWeave: Output height exceeds integer range.");
  vi.height *= 2;
...
int __stdcall DoubleWeaveFields::SetCacheHints(int cachehints, int frame_range) {
  (void)frame_range;
