  ...
  try {
    ...
    for (int type : types)
      for (int width : {8, 24, 40}) {
        ...
        compare(env, "StackHorizontal", {source, second});
        compare(env, "StackVertical", {source, second});
        compare(env, "StackHorizontal", {source, second, source});
        compare(env, "StackVertical", {source, second, source});
        const auto& vi = source->GetVideoInfo();
        if (vi.BitsPerComponent() == 8 && !vi.IsPlanarRGB() && !vi.IsPlanarRGBA() && !vi.IsYUVA())
          compare(env, "ShowFiveVersions", {source, second, source, second, source});
        AVSValue args[] = {source, second, source, second, source};
        auto five = env->Invoke("IFShowFiveVersions", AVSValue(args, 5)).AsClip();
