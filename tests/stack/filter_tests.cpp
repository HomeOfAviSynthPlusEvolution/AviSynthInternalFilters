...
#include "../../src/stack/kernel_adapter.h"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
using aif::filters::stack::allowed_cpu_flags;
...
void run_mode(Runtime& runtime, const char* plugin, const char* mode) {
  ...
  try {
    ...
    const int types[] = {VideoInfo::CS_Y8,    VideoInfo::CS_Y10,       VideoInfo::CS_Y16,      VideoInfo::CS_Y32,
                         VideoInfo::CS_YV12,  VideoInfo::CS_YUV444P10, VideoInfo::CS_YUV444PS, VideoInfo::CS_YUVA420P16,
                         VideoInfo::CS_RGBP,  VideoInfo::CS_RGBAP16,   VideoInfo::CS_RGBAPS,   VideoInfo::CS_BGR24,
                         VideoInfo::CS_BGR32, VideoInfo::CS_BGR48,     VideoInfo::CS_BGR64,    VideoInfo::CS_YUY2};
    for (int type : types)
      for (int width : {8, 24, 40}) {
        PClip source(new Sequence(env, type, width, 4, false)), second(new Sequence(env, type, width, 4, true));
        compare(reference_env, env, "StackHorizontal", {source, second});
        compare(reference_env, env, "StackVertical", {source, second});
        compare(reference_env, env, "StackHorizontal", {source, second, source});
        compare(reference_env, env, "StackVertical", {source, second, source});
        const auto& vi = source->GetVideoInfo();
        if (vi.BitsPerComponent() == 8 && !vi.IsPlanarRGB() && !vi.IsPlanarRGBA() && !vi.IsYUVA())
          compare(reference_env, env, "ShowFiveVersions", {source, second, source, second, source});
        AVSValue args[] = {source, second, source, second, source};
        auto five = env->Invoke("ShowFiveVersions", AVSValue(args, 5)).AsClip();
        PVideoFrame frame = five->GetFrame(2, env), sf = source->GetFrame(2, env), ss = second->GetFrame(2, env);
        if (vi.IsPlanar())
          for (int p : planes(vi)) {
            int bytes = vi.ComponentSize(), w = sf->GetRowSize(p) / bytes, h = sf->GetHeight(p);
            for (int c = 0; c < 5; ++c) {
              int x = (c / 2) * w + (c & 1 ? w / 2 : 0), y = (c & 1) ? h : 0;
              require(std::memcmp(frame->GetReadPtr(p) + y * frame->GetPitch(p) + x * bytes,
                                  (c & 1 ? ss : sf)->GetReadPtr(p), bytes) == 0,
                      "ShowFive plane placement");
            }
            const uint8_t* gap = frame->GetReadPtr(p) + h * frame->GetPitch(p);
            if (bytes == 1)
              require(*gap == 128, "ShowFive U8 gap");
            else if (bytes == 2)
              require(*reinterpret_cast<const uint16_t*>(gap) == (128u << (vi.BitsPerComponent() - 8)),
                      "ShowFive high-depth gap");
            else
              require(*reinterpret_cast<const float*>(gap) ==
                          (!vi.IsRGB() && (p == PLANAR_U || p == PLANAR_V) ? 0.f : 128.f / 255.f),
                      "ShowFive float gap");
          }
      }
