...
#include "../../src/mask/kernel_adapter.h"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
using aif::filters::mask::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_MASK_SSE2);
...
void run_mode(Runtime& runtime, const char* plugin, const char* mode) {
  ...
  try {
    ...
    const int types[] = {VideoInfo::CS_BGR32,   VideoInfo::CS_BGR64,   VideoInfo::CS_RGBAP,   VideoInfo::CS_RGBAP10,
                         VideoInfo::CS_RGBAP12, VideoInfo::CS_RGBAP14, VideoInfo::CS_RGBAP16, VideoInfo::CS_RGBAPS};
    for (int type : types)
      for (int width : {4, 16, 20, 65}) {
        PClip source(new Sequence(env, type, width, 4, false));
        compare(reference_env, env, "Mask", {source, source});
        for (double value : {-1.0, 0.0, 0.5, 128.0, 255.0, 65535.0})
          compare(reference_env, env, "ResetMask", {source, value});
        if (type != VideoInfo::CS_RGBAPS)
          for (int key : {0, 0x806040, 0xffffff})
            for (int tol : {0, 10, 255, -1, 256})
              compare(reference_env, env, "ColorKeyMask", {source, key, tol});
      }
    for (int type : {VideoInfo::CS_YUY2, VideoInfo::CS_YV12, VideoInfo::CS_YUV444P10, VideoInfo::CS_YUV444P16,
                     VideoInfo::CS_YUV444PS}) {
      PClip source(new Sequence(env, type, 20, 4, false));
      for (bool real : {false, true})
        compare(reference_env, env, "MaskHS", {source, 30.0, 220.0, 120.0, 10.0, false, real});
    }
    // Independent oracle for the corrected floating-point key lower bound.
    PClip black = env->Invoke("Eval", "BlankClip(width=4,height=2,pixel_type=\"RGBAPS\",colors=[0,0,0,1])").AsClip();
    const AVSValue keyargs[] = {black, 0x808080, 0};
    PClip keyed = env->Invoke("ColorKeyMask", AVSValue(keyargs, 3)).AsClip();
