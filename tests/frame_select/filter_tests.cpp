...
class SampleTimeline final : public GenericVideoFilter {
  int version_;
  int devices_;

public:
  SampleTimeline(PClip clip, int frames, int numerator = 25, int denominator = 1, int version = 8, int devices = 0)
      : GenericVideoFilter(clip), version_(version), devices_(devices) {
    vi.num_frames = frames;
    vi.fps_numerator = numerator;
    vi.fps_denominator = denominator;
    vi.num_audio_samples = vi.AudioSamplesFromFrames(frames);
  }
  static int16_t sample(int64_t position, int channel) { return int16_t((position * 17 + channel * 3) & 65535); }
  int __stdcall GetVersion() override { return version_; }
  int __stdcall SetCacheHints(int hint, int) override { return hint == CACHE_GET_DEV_TYPE ? devices_ : 0; }
  void __stdcall GetAudio(void* buffer, int64_t start, int64_t count, IScriptEnvironment*) override {
    ...
    for (int64_t i = 0; i < count; ++i)
      for (int channel = 0; channel < 2; ++channel)
        output[i * 2 + channel] = sample(start + i, channel);
  }
};

void check_range_audio(IScriptEnvironment* env) {
  PClip video(new Sequence(env, VideoInfo::CS_Y8, 16, 8, false));
  for (int numerator : {25, 30000}) {
    PClip source(new SampleTimeline(video, 17, numerator, numerator == 25 ? 1 : 1001));
    const auto& vi = source->GetVideoInfo();
    // Independent frame-to-sample floor; do not share the implementation's
    // FramesFromAudioSamples approximation or segment splitting logic.
    const auto sample_at_frame = [&](int64_t frame) {
      return frame * vi.audio_samples_per_second * vi.fps_denominator / vi.fps_numerator;
    };
    for (int offset : {0, 1})
      for (bool audio : {false, true}) {
        const AVSValue args[] = {source, 3, 2, offset, audio};
        PClip filter = env->Invoke("SelectRangeEvery", AVSValue(args, 5)).AsClip();
        for (int block : {1, 2, 4})
          for (int displacement : {-7, 0, 1}) {
            ++cases;
            const int64_t start = sample_at_frame(block * 2) + displacement;
            const int count = 37;
            std::vector<int16_t> actual(count * 2), split(count * 2);
            filter->GetAudio(actual.data(), start, count, env);
            filter->GetAudio(split.data(), start, 5, env);
            filter->GetAudio(split.data() + 10, start + 5, count - 5, env);
            require(actual == split, "SelectRangeEvery audio depends on request chunking");
            for (int i = 0; i < count; ++i) {
              const int64_t output_sample = start + i;
              int segment = 0;
              while (sample_at_frame((segment + 1) * 2) <= output_sample)
                ++segment;
              const int64_t input_sample = audio ? sample_at_frame(offset) + sample_at_frame(segment * 3) +
                                                       output_sample - sample_at_frame(segment * 2)
                                                 : output_sample;
              for (int channel = 0; channel < 2; ++channel) {
                if (actual[i * 2 + channel] != SampleTimeline::sample(input_sample, channel)) {
                  std::fprintf(stderr, "range audio mismatch: fps=%d offset=%d audio=%d start=%lld i=%d\n", numerator,
                               offset, audio, static_cast<long long>(start), i);
                  throw std::runtime_error("SelectRangeEvery audio differs from segment oracle");
                }
              }
            }
          }
...
void check_interleave_boundaries(IScriptEnvironment* env) {
  PClip video(new Sequence(env, VideoInfo::CS_Y8, 16, 8, false));
  const auto interleave = [&](PClip first, PClip second) {
    const AVSValue args[] = {first, second};
    return env->Invoke("Interleave", AVSValue(args, 2)).AsClip();
  };
  ++cases;
  PClip legacy(new SampleTimeline(video, 3, 25, 1, 4, 0));
  PClip current(new SampleTimeline(video, 5, 25, 1, 8, DEV_TYPE_CPU));
  PClip mixed = interleave(legacy, current);
  require(mixed->GetVideoInfo().num_frames == 10, "Interleave unequal lengths changed");
  require(mixed->SetCacheHints(CACHE_GET_DEV_TYPE, 0) == DEV_TYPE_CPU,
          "Interleave legacy child did not fall back to CPU");
  ++cases;
  bool rejected = false;
  try {
    PClip huge(new SampleTimeline(video, std::numeric_limits<int>::max()));
    interleave(huge, current);
  } catch (const AvisynthError&) {
    rejected = true;
  }
  require(rejected, "Interleave overflowing frame count was accepted");
  ++cases;
  rejected = false;
  try {
    PClip other_device(new SampleTimeline(video, 5, 25, 1, 8, 2));
    interleave(current, other_device);
  } catch (const AvisynthError&) {
    rejected = true;
  }
  require(rejected, "Interleave disjoint device types were accepted");
}
  ...
  try {
    ...
    for (int type : {VideoInfo::CS_BGR24, VideoInfo::CS_BGR32, VideoInfo::CS_YUY2, VideoInfo::CS_Y8, VideoInfo::CS_YV12,
      ...
      for (int width : {16, 66}) {
        ...
        for (const char* name : {"SelectEven", "SelectOdd"})
          compare(reference_env, env, name, {source});
        compare(reference_env, env, "SelectEvery", {source, 3, 0});
        compare(reference_env, env, "SelectEvery", {source, 3, 2, 0, 2});
        PClip second(new Sequence(env, type, width, 32, true));
        compare(reference_env, env, "Interleave", {source, second});
        for (bool audio : {false, true})
          for (int offset : {0, 1})
            compare(reference_env, env, "SelectRangeEvery", {source, 3, 2, offset, audio});
      }
    }
    check_range_audio(env);
    check_interleave_boundaries(env);
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::printf("%d runtime differential and boundary cases passed\n", cases);
  } catch (const AvisynthError& e) {
