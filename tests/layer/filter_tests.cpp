  ...
  try {
    ...
    for (int type : types)
      for (int width : {4, 20, 68}) {
        ...
        PClip overlay(new Sequence(env, type, width, 8, true));
        compare(reference_env, env, "Subtract", {source, overlay});
        for (const char* op : {"Add", "Mul", "Subtract", "Fast", "Lighten", "Darken"})
          for (double opacity : {0.0, 0.5, 1.0})
            for (int x : {-2, 0, 2})
              compare(reference_env, env, "Layer", {source, overlay, op, AVSValue(), x, 2, 10, true, opacity, "mpeg2"});
      }
    for (int type : {VideoInfo::CS_BGR32, VideoInfo::CS_YUVA420P16, VideoInfo::CS_YUVA444PS}) {
      PClip source(new Sequence(env, type, 20, 8, false));
      PClip overlay(new Sequence(env, type, 24, 12, true));
      for (const char* op : {"Add", "Mul", "Subtract"})
        for (const char* placement : {"mpeg1", "mpeg2"})
          for (int x : {-7, 1, 1000000, -2147483647})
            compare(reference_env, env, "Layer", {source, overlay, op, AVSValue(), x, -3, 255, false, 0.37, placement});
    }
