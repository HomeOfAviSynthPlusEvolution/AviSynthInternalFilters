...
using aif::filters::channel_display::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_CHANNEL_DISPLAY_SSE2);
...
void run_mode(Runtime& runtime, const char* plugin, const char* mode) {
  ...
  try {
    ...
    const int types[] = {VideoInfo::CS_Y8,         VideoInfo::CS_Y10,   VideoInfo::CS_Y16,       VideoInfo::CS_Y32,
                         VideoInfo::CS_YV12,       VideoInfo::CS_YV16,  VideoInfo::CS_YUV444P10, VideoInfo::CS_YUV444PS,
    ...
    for (int type : types)
      for (int width : {16, 36})
        for (int height : {4, 8}) {
          ...
          const auto& vi = source->GetVideoInfo();
          std::vector<const char*> names = vi.IsRGB() ? std::vector<const char*>{"ShowRed", "ShowGreen", "ShowBlue"}
                                                      : std::vector<const char*>{"ShowY"};
          if (!vi.IsRGB() && !vi.IsY()) {
            names.push_back("ShowU");
            names.push_back("ShowV");
          }
