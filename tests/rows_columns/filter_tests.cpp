...
using aif::filters::rows_columns::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_ROWS_COLUMNS_SSE2);
...
void compare(IScriptEnvironment* reference_env, IScriptEnvironment* env, const char* name,
  ...
  require(out.width == ref.width && out.height == ref.height && out.pixel_type == ref.pixel_type &&
              out.num_frames == ref.num_frames && out.fps_numerator == ref.fps_numerator &&
  ...
  for (int n : {out.num_frames - 1, 0, out.num_frames / 2, 0}) {
    ...
    int error = 0;
    require(env->propGetInt(env->getFramePropsRO(b), "AIFTest", 0, &error) ==
                    env->propGetInt(env->getFramePropsRO(a), "AIFTest", 0, nullptr) &&
                !error,
