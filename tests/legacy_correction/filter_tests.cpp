...
void run_mode(Runtime& runtime, const char* plugin, const char* mode) {
  ...
  try {
    ...
    for (int width : {2, 6, 32, 66})
      for (int height : {1, 3, 4, 7, 32}) {
        PClip source(new Sequence(env, VideoInfo::CS_YUY2, width, height, false));
        compare(reference_env, env, "FixBrokenChromaUpsampling", {source});
        for (int cutoff : {-2, 0, 1, 16, 40})
          compare(reference_env, env, "PeculiarBlend", {source, cutoff});
        compare(reference_env, env, "FixLuminance", {source, height / 2, 1.0});
        compare(reference_env, env, "FixLuminance", {source, -1, -1.0});
        for (int skew : {0, 2, -width + 2})
          compare(reference_env, env, "SkewRows", {source, skew});
      }
    for (int type :
         {VideoInfo::CS_BGR24, VideoInfo::CS_BGR32, VideoInfo::CS_Y8, VideoInfo::CS_Y16, VideoInfo::CS_Y32}) {
      PClip source(new Sequence(env, type, 17, 7, false));
      for (int skew : {-16, -3, 0, 7, 31})
        compare(reference_env, env, "SkewRows", {source, skew});
    }
...
void boundary_cases(Runtime& runtime, const char* plugin) {
  ...
  auto* env = holder.env;
  env->Invoke("LoadPlugin", plugin);
  PClip source(new Sequence(env, VideoInfo::CS_YUY2, 6, 3, false));
  const auto original = snapshot(source->GetFrame(0, env), source->GetVideoInfo());
  const auto reject = [&](const char* name, std::vector<AVSValue> args) {
    bool rejected = false;
    try {
    ...
    require(rejected, "invalid legacy arguments accepted");
    ++cases;
  };
  for (double slope :
       {0., .01, std::numeric_limits<double>::infinity(), std::numeric_limits<double>::quiet_NaN(), double(INT32_MAX)})
    reject("FixLuminance", {source, 1, slope});
  for (int vertex : {INT32_MIN, INT32_MAX}) {
    AVSValue args[] = {source, vertex, 1.0};
    PClip f = env->Invoke("FixLuminance", AVSValue(args, 3)).AsClip();
    auto expected = original;
    if (vertex == INT32_MAX)
      for (size_t x = 0; x < expected.size(); x += 2)
        expected[x] = 0;
    require(snapshot(f->GetFrame(0, env), f->GetVideoInfo()) == expected, "extreme vertex result incorrect");
    ++cases;
  }
  for (int cutoff : {INT32_MIN, INT32_MAX}) {
    AVSValue args[] = {source, cutoff};
    PClip f = env->Invoke("PeculiarBlend", AVSValue(args, 2)).AsClip();
    const auto expected = snapshot(source->GetFrame(cutoff == INT32_MIN ? 0 : 1, env), source->GetVideoInfo());
    require(snapshot(f->GetFrame(0, env), f->GetVideoInfo()) == expected, "extreme cutoff result incorrect");
    ++cases;
  }
  for (int skew : {-6, INT32_MIN, INT32_MAX, 1})
    reject("SkewRows", {source, skew});
  PClip planar(new Sequence(env, VideoInfo::CS_YV24, 6, 3, false));
  reject("SkewRows", {planar, 0});
  reject("FixBrokenChromaUpsampling", {planar});
  reject("PeculiarBlend", {planar, 0});
  reject("FixLuminance", {planar, 0, 1.});
  // Two rows have no complete chroma pair beginning on row 1.
  PClip tiny(new Sequence(env, VideoInfo::CS_YUY2, 2, 2, false));
  PClip fixed = env->Invoke("FixBrokenChromaUpsampling", tiny).AsClip();
  require(snapshot(fixed->GetFrame(0, env), fixed->GetVideoInfo()) ==
              snapshot(tiny->GetFrame(0, env), tiny->GetVideoInfo()),
          "incomplete chroma row pair changed");
  ++cases;
  require(snapshot(source->GetFrame(0, env), source->GetVideoInfo()) == original, "boundary test modified source");
  std::printf("18 legacy boundary cases passed\n");
}
} // namespace
