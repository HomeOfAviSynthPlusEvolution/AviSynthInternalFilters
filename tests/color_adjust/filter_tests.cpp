...
#include "../../src/color_adjust/kernel_adapter.h"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
static_assert(color_cpu_flags(0) == 0);
static_assert(color_cpu_flags(CPUF_SSE2) == AIF_COLOR_ADJUST_SSE2);
static_assert(color_cpu_flags(base_flags) == (1 | 2 | 8));
static_assert(color_cpu_flags(avx2_flags) == (1 | 2 | 8 | 16));
static_assert(color_cpu_flags(avx3_flags) == (1 | 2 | 8 | 16 | 32));
static_assert(color_cpu_flags(dl_flags) == (1 | 2 | 8 | 16 | 32 | 64));
static_assert(color_cpu_flags(dl_flags | CPUF_AVX512BF16) == (1 | 2 | 8 | 16 | 32 | 64 | 128));
static_assert(color_cpu_flags(dl_flags | CPUF_AVX512BF16 | CPUF_AVX512FP16) == (1 | 2 | 8 | 16 | 32 | 64 | 128 | 256));
static_assert(color_cpu_flags(avx2_flags & ~uint64_t(CPUF_FMA3)) == (1 | 2 | 8));
static_assert(color_cpu_flags(avx3_flags & ~uint64_t(CPUF_AVX512BW)) == (1 | 2 | 8 | 16));
#endif
...
public:
  Sequence(IScriptEnvironment* env, int type, int width, int height, bool stable) {
    ...
    for (int n = 0; n < vi_.num_frames; ++n) {
      ...
      env->propSetInt(env->getFramePropsRW(f), "_ChromaLocation", 0, PROPAPPENDMODE_REPLACE);
      frames_.push_back(f);
    }
  }
...
void run_mode(Runtime& runtime, const char* plugin, const char* mode) {
  ...
  try {
    ...
    const int types[] = {VideoInfo::CS_Y8,        VideoInfo::CS_Y10,       VideoInfo::CS_Y16,
                         VideoInfo::CS_Y32,       VideoInfo::CS_YV12,      VideoInfo::CS_YUY2,
                         VideoInfo::CS_YUV444P10, VideoInfo::CS_YUV444P12, VideoInfo::CS_YUVA420P16,
                         VideoInfo::CS_YUV444PS,  VideoInfo::CS_RGBP,      VideoInfo::CS_RGBAP10,
                         VideoInfo::CS_RGBAP16,   VideoInfo::CS_RGBAPS,    VideoInfo::CS_BGR24,
                         VideoInfo::CS_BGR32,     VideoInfo::CS_BGR48,     VideoInfo::CS_BGR64};
    for (int type : types)
      for (int width : {16, 36}) {
        PClip source(new Sequence(env, type, width, 8, false));
        const auto& vi = source->GetVideoInfo();
        float max = vi.ComponentSize() == 4 ? 1.0f : float((1 << vi.BitsPerComponent()) - 1);
        for (bool dither : {false, true}) {
          if (dither && vi.BitsPerComponent() > 10 && vi.BitsPerComponent() != 32)
            continue;
          compare(reference_env, env, "Levels",
                  {source, 0.05f * max, 1.2f, .9f * max, .1f * max, .95f * max, false, dither});
          compare(reference_env, env, "Levels", {source, 0.0f, 1.0f, max, 0.0f, max, true, dither});
          if (vi.IsRGB()) {
            compare(reference_env, env, "RGBAdjust",
                    {source, 1.1f, .8f, 1.2f, .7f, 2.f, -1.f, 3.f, 4.f, .9f, 1.1f, 1.2f, .8f, false, dither});
          } else if (!vi.IsY()) {
            compare(reference_env, env, "Tweak",
                    {source, 27.f, 1.1f, 2.f, .9f, true, false, 0.f, 360.f, 150.f, 0.f, 16.f, dither, false});
            compare(reference_env, env, "Tweak",
                    {source, -15.f, .7f, -2.f, 1.2f, false, false, 20.f, 200.f, 100.f, 10.f, 8.f, dither, true});
          } else {
            compare(reference_env, env, "Tweak",
                    {source, 0.f, 1.f, 3.f, .9f, true, false, 0.f, 360.f, 150.f, 0.f, 16.f, dither, false});
          }
        }
        if (!vi.IsRGB()) {
          compare(reference_env, env, "ColorYUV",
                  {source, 12.f, -3.f, 7.f, 5.f, -8.f, 2.f, 4.f, 0.f, 3.f, -2.f, 0.f, 4.f});
          compare(reference_env, env, "ColorYUV",
                  {source, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, "TV->PC"});
        }
      }
