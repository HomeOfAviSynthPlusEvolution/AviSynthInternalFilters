...
#include "../../src/merge/kernel_adapter.h"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
using aif::filters::merge::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_MERGE_SSE2);
...
void run_mode(Runtime& runtime, const char* plugin, const char* mode) {
  ...
  try {
    ...
    const int types[] = {VideoInfo::CS_Y8,         VideoInfo::CS_Y10,   VideoInfo::CS_Y16,       VideoInfo::CS_Y32,
                         VideoInfo::CS_YV12,       VideoInfo::CS_YV411, VideoInfo::CS_YUV444P10, VideoInfo::CS_YUV444PS,
                         VideoInfo::CS_YUVA420P16, VideoInfo::CS_RGBP,  VideoInfo::CS_RGBAP16,   VideoInfo::CS_RGBAPS,
    ...
    for (int type : types)
      for (int width : {16, 36}) {
        ...
        for (float weight : {-1.f, 0.f, .0038f, .0039f, .25f, .4961f, .497f, .5f, .5039f, .75f, .9961f, 1.f, 2.f}) {
          compare(reference_env, env, "Merge", {source, second, weight});
          if (!source->GetVideoInfo().IsRGB()) {
            compare(reference_env, env, "MergeLuma", {source, second, weight});
            compare(reference_env, env, "MergeChroma", {source, second, weight});
          }
        }
      }
