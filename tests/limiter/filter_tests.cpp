...
#include "../../src/limiter/kernel_adapter.h"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
using aif::filters::limiter::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_LIMITER_SSE2);
      ...
      for (int width : {4, 16, 20, 64, 68})
        for (int height : {4, 8, 20}) {
          ...
          if (source->GetVideoInfo().IsRGB())
            continue;
          compare(reference_env, env, "Limiter", {source});
          compare(reference_env, env, "Limiter", {source, 16.5, 230.5, 10.5, 240.5, AVSValue(), true});
          const auto& vi = source->GetVideoInfo();
          if (vi.IsYUY2() || vi.Is444() || vi.Is420())
            for (const char* show : {"luma", "luma_grey", "chroma", "chroma_grey"}) {
              // Mixed out-of-range 420 luma blocks have an inherited x+2 indexing bug.
              // Its corrected x+1 behavior is tested directly in kernel_tests.
              if (vi.Is420() && std::strcmp(show, "luma") == 0)
                continue;
              compare(reference_env, env, "Limiter", {source, AVSValue(), AVSValue(), AVSValue(), AVSValue(), show});
            }
        }
    for (int type : {VideoInfo::CS_YV12, VideoInfo::CS_YUV444PS}) {
      PClip source(new Sequence(env, type, 8, 4, false));
      for (const auto& limits :
           std::vector<std::vector<AVSValue>>{{source, 240.0, 16.0}, {source, 0.0, 255.0, 240.0, 16.0}}) {
        for (auto* test_env : {reference_env, env}) {
          bool rejected = false;
          try {
            test_env->Invoke("Limiter", AVSValue(limits.data(), int(limits.size())));
          } catch (const AvisynthError&) {
            rejected = true;
          }
          require(rejected, "reversed limits accepted");
        }
      }
    }
