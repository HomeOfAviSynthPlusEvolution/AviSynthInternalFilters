  ...
  try {
      ...
      for (int width : {4, 16, 20, 64, 68})
        for (int height : {4, 8, 20}) {
          ...
          if (source->GetVideoInfo().IsRGB())
            continue;
          compare(env, "Limiter", {source});
          compare(env, "Limiter", {source, 16.5, 230.5, 10.5, 240.5, AVSValue(), true});
          const auto& vi = source->GetVideoInfo();
          if (vi.IsYUY2() || vi.Is444() || vi.Is420())
            for (const char* show : {"luma", "luma_grey", "chroma", "chroma_grey"}) {
              ...
              if (vi.Is420() && std::strcmp(show, "luma") == 0)
                continue;
              compare(env, "Limiter", {source, AVSValue(), AVSValue(), AVSValue(), AVSValue(), show});
            }
        }
    for (int type : {VideoInfo::CS_YV12, VideoInfo::CS_YUV444PS}) {
      ...
      for (const auto& limits :
        ...
        for (const char* name : {"Limiter", "IFLimiter"}) {
          bool rejected = false;
          try {
            env->Invoke(name, AVSValue(limits.data(), int(limits.size())));
