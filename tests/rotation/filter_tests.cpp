...
#define NOMINMAX
#include <avisynth.h>
#include "../../src/rotation/kernel_adapter.h"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
using aif::filters::rotation::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_ROTATION_SSE2);
    ...
    for (int type : types)
      for (int width : {4, 16, 20, 64, 68})
        for (int height : {4, 8, 20}) {
          ...
          for (const char* name : {"TurnLeft", "TurnRight", "Turn180", "FlipHorizontal", "FlipVertical"})
            compare(env, name, {source});
        }
    for (const char* name : {"TurnLeft", "TurnRight"}) {
      PClip source(new Sequence(env, VideoInfo::CS_YUY2, 8, 3, false));
      bool rejected = false;
      try {
        env->Invoke((std::string("IF") + name).c_str(), source);
      } catch (const AvisynthError&) {
        rejected = true;
      }
      require(rejected, "odd YUY2 height accepted");
    }
