...
using aif::filters::planes::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_PLANES_SSE2);
...
void compare(IScriptEnvironment* reference_env, IScriptEnvironment* env, const char* name,
  ...
  for (int n : {4, 0, 2, 2}) {
    ...
    if (out.IsYUVA() && (std::strcmp(name, "UToY") == 0 || std::strcmp(name, "VToY") == 0)) {
      aa.resize(aa.size() - a->GetRowSize(PLANAR_A) * a->GetHeight(PLANAR_A));
      bb.resize(bb.size() - b->GetRowSize(PLANAR_A) * b->GetHeight(PLANAR_A));
    }
    bool equal = aa == bb;
    if (!equal) {
      size_t first = 0;
