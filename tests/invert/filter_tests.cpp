...
#include "../../src/invert/kernel_adapter.h"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
using aif::filters::invert::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_INVERT_SSE2);
    ...
    for (int type : types)
      for (int width : {4, 16, 20, 64, 68})
        for (int height : {4, 8, 20}) {
          ...
          compare(reference_env, env, "Invert", {source});
          for (const char* channels : {"", "Y", "UV", "RGB", "A", "bGyV", "?"})
            compare(reference_env, env, "Invert", {source, channels});
        }
