...
void compare(IScriptEnvironment* env, const char* name, const std::vector<AVSValue>& args) {
  ...
  for (int n : {4, 0, 2, 2}) {
    ...
    if (!equal && vi.ComponentSize() == 4) {
      equal = aa.size() == bb.size();
      for (size_t i = 0; equal && i < aa.size(); i += 4) {
        float x, y;
        std::memcpy(&x, aa.data() + i, 4);
        std::memcpy(&y, bb.data() + i, 4);
        equal = std::abs(x - y) <= 1e-6f * std::max(1.0f, std::abs(x));
      }
    }
    if (!equal) {
      size_t first = 0;
  ...
  try {
    ...
    for (int type : types)
      for (bool stable : {false, true}) {
        PClip source(new Sequence(env, type, 64, 8, stable));
    ...
    for (const char* script : bad_scripts) {
      ...
      require(rejected, "invalid filter arguments accepted");
      ++cases;
    }
