...
#include "../../src/rgb_merge/kernel_adapter.h"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
using aif::filters::rgb_merge::allowed_cpu_flags;
...
static_assert(allowed_cpu_flags(CPUF_SSE2) == AIF_RGB_MERGE_SSE2);
...
void compare(IScriptEnvironment* reference_env, IScriptEnvironment* env, const char* name,
  ...
  for (int n : {4, 0, 2, 2}) {
    ...
    for (const char* key : {"_Matrix", "_ChromaLocation"}) {
      int ea = 0, eb = 0;
...
void run_mode(Runtime& runtime, const char* plugin, const char* mode) {
  ...
  try {
    ...
    const int types[] = {VideoInfo::CS_Y8,      VideoInfo::CS_Y10,       VideoInfo::CS_Y16,      VideoInfo::CS_Y32,
                         VideoInfo::CS_YV12,    VideoInfo::CS_YUV444P10, VideoInfo::CS_YUV444PS, VideoInfo::CS_RGBP,
                         VideoInfo::CS_RGBAP16, VideoInfo::CS_RGBAPS,    VideoInfo::CS_BGR24,    VideoInfo::CS_BGR32,
                         VideoInfo::CS_BGR48,   VideoInfo::CS_BGR64};
    for (int type : types)
      for (int width : {4, 18, 36}) {
        PClip r(new Sequence(env, type, width, 4, false)), g(new Sequence(env, type, width, 4, true)),
            b(new Sequence(env, type, width, 4, false));
        const auto& vi = r->GetVideoInfo();
        for (const char* output : {"", "rgb", "rgbp", "rgbap"}) {
          compare(reference_env, env, "MergeRGB", {r, g, b, output});
          if (!vi.IsRGB() || vi.NumComponents() == 4)
            for (const char* ao : {"", "rgb", "rgbap"})
              compare(reference_env, env, "MergeARGB", {g, r, g, b, ao});
        }
        if (vi.BitsPerComponent() == 8 || vi.BitsPerComponent() == 16) {
          for (const char* output :
               {vi.BitsPerComponent() == 8 ? "RGB24" : "RGB48", vi.BitsPerComponent() == 8 ? "RGB32" : "RGB64"})
            compare(reference_env, env, "MergeRGB", {r, g, b, output});
          PClip grey = env->Invoke("ExtractR", env->Invoke("ConvertToPlanarRGB", r)).AsClip();
          compare(reference_env, env, "MergeRGB", {r, grey, b, "rgb"});
          compare(reference_env, env, "MergeRGB", {grey, g, b});
        }
      }
    // YUY2 must use its luma samples and top-down orientation, as planar Y does.
    PClip packed(new Sequence(env, VideoInfo::CS_YUY2, 18, 4, false));
    PClip y = env->Invoke("ExtractY", packed).AsClip();
    for (const char* output : {"RGB24", "RGB32", "rgbp", "rgbap"}) {
      AVSValue args[] = {packed, packed, packed, output}, expected[] = {y, y, y, output};
      auto actual = env->Invoke("MergeRGB", AVSValue(args, 4)).AsClip();
      const auto reference_args =
          reference_arguments(std::vector<AVSValue>(expected, expected + 4), env, reference_env);
      auto reference = reference_env->Invoke("MergeRGB", AVSValue(reference_args.data(), 4)).AsClip();
      require(snapshot(actual->GetFrame(2, env), actual->GetVideoInfo()) ==
                  snapshot(reference->GetFrame(2, reference_env), reference->GetVideoInfo()),
              "YUY2 luma/orientation regression");
    }
